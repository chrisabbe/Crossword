...
const puzzleFile=urlParams.get('puzzle')||'puzzle1.json';

// load JSON from raw GitHub
const baseRaw='https://raw.githubusercontent.com/chrisabbe/crossword/main/';
fetch(baseRaw+puzzleFile)
 .then(r=>r.json())
 .then(data=>{document.getElementById('title').textContent=data.title||'Daily Crossword';
...
function buildPuzzle(data){
 ...
 const saveKey='xwd:'+puzzleFile,answers=JSON.parse(localStorage.getItem(saveKey)||'{}');
 gridEl.innerHTML='';
